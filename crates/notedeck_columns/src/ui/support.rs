...
use notedeck::{NamedFontFamily, NotedeckTextStyle, tr};
...
impl<'a> SupportView<'a> {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui) {
        padding(8.0, ui, |ui| {
            ...
            ui.add(Label::new(RichText::new(tr!("Running into a bug?")).font(font)));
            ui.label(RichText::new(tr!("Step 1")).text_style(NotedeckTextStyle::Heading3.text_style()));
            padding(8.0, ui, |ui| {
                ui.label(tr!("Open your default email client to get help from the Damus team"));
            ...
            if let Some(logs) = self.support.get_most_recent_log() {
                ui.label(
                    RichText::new(tr!("Step 2")).text_style(NotedeckTextStyle::Heading3.text_style()),
                );
                let size = vec2(80.0, 40.0);
                let copy_button = Button::new(RichText::new(tr!("Copy")).size(
                    notedeck::fonts::get_font_size(ui.ctx(), &NotedeckTextStyle::Body),
                ))
                .fill(PINK)
                .min_size(size);
                padding(8.0, ui, |ui| {
                    ui.add(Label::new(RichText::new(tr!("Press the button below to copy your most recent logs to your system's clipboard. Then paste it into your email."))).wrap());
                    ui.allocate_ui_with_layout(size, Layout::top_down(egui::Align::Center), |ui| {
                ...
                ui.label(
                    egui::RichText::new(tr!("ERROR: Could not find logs on system"))
...
fn open_email_button(font_size: f32, size: egui::Vec2) -> impl egui::Widget {
    Button::new(RichText::new(tr!("Open Email")).size(font_size))
        .fill(PINK)
        .min_size(size)
}
